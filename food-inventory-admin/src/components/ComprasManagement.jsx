...
import { AlertTriangle, Clock, PlusCircle, Trash2, CalendarIcon, Package, CheckCircle, XCircle, Plus, RefreshCw } from 'lucide-react';
...
export default function ComprasManagement() {
  ...
  const debouncedSupplierSearch = useDebounce(po.newSupplierName, 300);
  ...
  useEffect(() => {
    if (debouncedSupplierSearch && !po.supplierId) {
      fetchApi(`/suppliers?search=${debouncedSupplierSearch}`).then(res => {
        if (res.success && res.data.length > 0) {
          setSupplierSearchResults(res.data);
  ...
  }, [debouncedSupplierSearch, po.supplierId]);

  useEffect(() => {
    if (debouncedSearch.length > 2) {
      fetchApi(`/inventory?search=${debouncedSearch}`).then(res => {
        if (res.success) {
          setProductSearchResults(res.data.map(item => ({ value: item.productSku, label: `${item.productName} (${item.productSku})`, ...item })));
        }
      });
    }
  }, [debouncedSearch]);
  ...
  const handleSupplierNameChange = (name) => {
    setPo(prev => ({ ...prev, newSupplierName: name, supplierId: '' }));
    if (name.length > 1 && !isSupplierPopoverOpen) {
      setIsSupplierPopoverOpen(true);
    }
  };

  const handleSupplierSelect = (supplier) => {
    const rifParts = supplier.taxInfo.rif.split('-');
    setPo(prev => ({
      ...prev,
      supplierId: supplier._id,
      newSupplierName: supplier.name,
      rifPrefix: rifParts[0] || 'J',
      newSupplierRif: rifParts[1] || '',
      newSupplierContactName: supplier.contacts[0]?.name || '',
      newSupplierContactPhone: supplier.contacts[0]?.phone || '',
      newSupplierContactEmail: supplier.contacts[0]?.email || '',
    }));
  ...
  const handleAddProductToPo = (productSku) => {
    const product = productSearchResults.find(p => p.value === productSku);
    if (product && !po.items.some(item => item.productSku === product.productSku)) {
      const newItem = {
  ...
  const handlePoSubmit = async () => {
    if (!po.newSupplierName || !po.newSupplierRif || !po.newSupplierContactName) {
      toast.error('Error de Validación', { description: 'Debe completar Nombre de la Empresa, RIF y Nombre del Vendedor.' });
      return;
    }

    if (!po.supplierId) {
      const fullRif = `${po.rifPrefix}-${po.newSupplierRif}`;
    ...
    for (const item of po.items) {
      if (item.isPerishable && (!item.lotNumber || !item.expirationDate)) {
        toast.error('Error de Validación', { description: `Para el producto perecedero "${item.productName}", debe especificar lote y vencimiento.` });
        return;
      }
    }
    ...
    if (po.supplierId) {
      ...
      dto.newSupplierName = po.newSupplierName;
      dto.newSupplierRif = `${po.rifPrefix}-${po.newSupplierRif}`;
      dto.newSupplierContactName = po.newSupplierContactName;
      dto.newSupplierContactPhone = po.newSupplierContactPhone;
      dto.newSupplierContactEmail = po.newSupplierContactEmail;
    }
              ...
              <div className="space-y-6 p-1">
                <div className="p-4 border rounded-lg space-y-4">
                  ...
                  <div className="space-y-2">
                    <Label>Nombre de la Empresa</Label>
                    <Input
                      placeholder="Buscar o crear un nuevo proveedor..."
                      value={po.newSupplierName}
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <Label>RIF</Label>
                      <div className="flex space-x-2">
                        <Select value={po.rifPrefix} onValueChange={(val) => updatePoField('rifPrefix', val)} disabled={!!po.supplierId}>
                          <SelectTrigger className="w-[80px]"><SelectValue /></SelectTrigger>
                          <SelectContent>{['J', 'V', 'E', 'G', 'P', 'N'].map(p => <SelectItem key={p} value={p}>{p}</SelectItem>)}</SelectContent>
                        </Select>
                        <Input value={po.newSupplierRif} onChange={e => updatePoField('newSupplierRif', e.target.value)} placeholder="12345678-9" disabled={!!po.supplierId} />
                      </div>
                    </div>
                    <div className="space-y-2">
                      <Label>Nombre del Vendedor</Label>
                      <Input value={po.newSupplierContactName} onChange={e => updatePoField('newSupplierContactName', e.target.value)} disabled={!!po.supplierId} />
                    </div>
                    <div className="space-y-2"><Label>Teléfono del Vendedor</Label><Input value={po.newSupplierContactPhone} onChange={e => updatePoField('newSupplierContactPhone', e.target.value)} disabled={!!po.supplierId} /></div>
                    <div className="space-y-2"><Label>Email del Vendedor</Label><Input value={po.newSupplierContactEmail} onChange={e => updatePoField('newSupplierContactEmail', e.target.value)} disabled={!!po.supplierId} /></div>
                  </div>
                </div>
                ...
                <div className="p-4 border rounded-lg space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <Combobox options={productSearchResults} value={productSearch} onInputChange={setProductSearch} onChange={handleAddProductToPo} placeholder="Buscar producto por nombre o SKU..." />
                  </div>
                  <Table>
                    ...
                    <TableBody>
                      {po.items.map((item, index) => (
                        <TableRow key={index}>
                          ...
                          <TableCell><Input type="number" value={item.quantity} onChange={e => updateItemField(index, 'quantity', Number(e.target.value))} className="w-24" /></TableCell>
                          <TableCell><Input type="number" value={item.costPrice} onChange={e => updateItemField(index, 'costPrice', Number(e.target.value))} className="w-32" /></TableCell>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <Popover>
                      ...
                      <PopoverContent className="w-auto p-0"><Calendar mode="single" selected={po.purchaseDate} onSelect={(date) => updatePoField('purchaseDate', date)} initialFocus /></PopoverContent>
                    </Popover>
                  </div>
                  <div className="space-y-2"><Label>Notas</Label><Textarea value={po.notes} onChange={e => updatePoField('notes', e.target.value)} /></div>
                </div>
              </div>
