...
const ReservationForm = ({ reservation, initialDate, onClose, isEmbedded = false, initialCustomer = null }) => {
  ...
  useEffect(() => {
    if (reservation) {
      ...
      setFormData(prev => ({
        ...prev,
        guestName: initialCustomer.name || '',
        guestPhone: initialCustomer.phone || '',
        date: initialDate || prev.date
      }));
    } else if (initialDate) {
      setFormData(prev => ({ ...prev, date: initialDate }));
    }
  }, [reservation, initialDate, initialCustomer]);
  ...
  const footerContent = (
    <div className="flex justify-end gap-2 w-full">
      ...
      <Button onClick={handleSubmit} disabled={loading} className={isEmbedded ? "w-full" : ""}>
        {loading ? (
          <>
    ...
    return (
      <div className="flex flex-col h-full bg-background">
        <div className="p-4 border-b flex-shrink-0">
          <h2 className="text-lg font-semibold">{isEditing ? 'Editar Reserva' : 'Detalles de Reserva'}</h2>
        </div>
