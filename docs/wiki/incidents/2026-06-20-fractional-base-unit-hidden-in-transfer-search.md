...
El mismo patrón (umbral entero sobre `availableQuantity` en unidad base fraccional) puede morder en otros consumidores: **POS** (ocultar productos vendibles a granel), **alertas de stock bajo** y **reportes de inventario**. Conviene auditar otros `>= 1` / `minAvailable` / `minAvailableQuantity` en el código antes de que reaparezca con otro tenant.
