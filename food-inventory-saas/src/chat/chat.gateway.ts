...
export class ChatGateway {
  ...
  handleConnection(client: Socket, ...args: any[]) {
    ...
    if (tenantId) {
      this.logger.log(`Client ${client.id} connected and joined room for tenant ${tenantId}`);
  ...
  async handleMessage(
    ...
    const tenantId = Array.from(client.rooms).find(room => room !== client.id);
    ...
    if (tenantId) {
      ...
      this.logger.error(`Could not find tenantId for client ${client.id}. Message not sent.`);
      client.emit('error', 'Authentication error: Missing tenantId. Cannot send message.');
    }
  }
}
