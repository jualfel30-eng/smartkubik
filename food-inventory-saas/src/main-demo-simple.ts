  ...
  getSchemas() {
    return {
      ...
      schemas: {
        Product: {
          description:
            "Productos con variantes, proveedores y configuración para alimentos perecederos",
          features: [
            "SKU único",
            "Variantes múltiples",
            "Proveedores",
...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    ...
    .setDescription(
      "Vista previa de la arquitectura del sistema de inventario alimentario para Venezuela",
    )
  ...
  console.log(
    `📋 Demo Endpoints: http://localhost:${port}/api/v1/demo/endpoints`,
  );
}
