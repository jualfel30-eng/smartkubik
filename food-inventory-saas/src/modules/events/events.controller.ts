...
export class EventsController {
  ...
  @ApiQuery({ name: 'start', required: false, description: 'Fecha de inicio para filtrar eventos (ISO 8601)' })
  @ApiQuery({ name: 'end', required: false, description: 'Fecha de fin para filtrar eventos (ISO 8601)' })
  @ApiResponse({ status: 200, description: 'Lista de eventos.' })
  async findAll(@Request() req, @Query('start') start?: string, @Query('end') end?: string) {
    try {
