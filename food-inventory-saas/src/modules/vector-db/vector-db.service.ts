...
export class VectorDbService implements OnModuleInit {
  ...
  async onModuleInit() {
    this.logger.log(
      "VectorDbService initialized. Pinecone client will be created on first use.",
    );
  }
  ...
  private async ensureVectorStoreInitialized(): Promise<void> {
    ...
    this.logger.log(
      "First use: Initializing Pinecone client and vector store...",
    );

    const apiKeySetting =
      await this.superAdminService.getSetting("PINECONE_API_KEY");
    const environmentSetting = await this.superAdminService.getSetting(
    ...
    if (!apiKey || !environment) {
      this.logger.error(
        "Pinecone API Key or Environment is not configured in Super Admin settings.",
      );
    ...
    const indexDescription =
      await this.pineconeClient.describeIndex(PINECONE_INDEX_NAME);
    const indexDimension = indexDescription.dimension;

    if (!indexDimension || Number.isNaN(Number(indexDimension))) {
      this.logger.error(
        `Unable to determine dimension for Pinecone index '${PINECONE_INDEX_NAME}'.`,
      );
      throw new InternalServerErrorException(
        "Pinecone index dimension is not available.",
      );
    }

    this.indexDimension = Number(indexDimension);
    this.logger.log(
      `Using Pinecone index '${PINECONE_INDEX_NAME}' with dimension ${this.indexDimension}.`,
    );
    ...
    this.logger.log(
      "Pinecone client and vector store initialized successfully.",
    );
  }
  ...
  async addDocuments(documents: Document[]) {
    ...
    this.logger.log(
      `Added ${documents.length} documents to Pinecone index '${PINECONE_INDEX_NAME}'.`,
    );
  }
  ...
  async deleteDocuments(source: string, tenantId: string): Promise<void> {
    await this.ensureVectorStoreInitialized();
    this.logger.log(
      `Deleting documents from source '${source}' for tenant '${tenantId}'...`,
    );
