...
export class PayablesController {
  ...
  @ApiResponse({
    status: 201,
    description: "La cuenta por pagar ha sido creada exitosamente.",
  })
  ...
  @Permissions("payables_read")
  @ApiOperation({
    summary: "Obtener todas las cuentas por pagar del tenant (paginadas)",
  })
  @ApiResponse({
    status: 200,
  ...
  @Permissions("payables_update")
  @ApiOperation({
    summary: "Migrar payables de draft a open (temporal migration endpoint)",
  })
  async migrateDraftToOpen(@Request() req) {
    try {
      const result = await this.payablesService.migrateDraftToOpen(
        req.user.tenantId,
      );
      return {
        success: true,
