...
export class ShiftsService {
  ...
  async findActiveShift(userId: string, tenantId: string): Promise<ShiftDocument | null> {
  ...
  async clockOut(userId: string, tenantId: string): Promise<ShiftDocument> {
    const activeShift = await this.findActiveShift(userId, tenantId);
    if (!activeShift) {
      throw new NotFoundException('No se encontró un turno activo para este usuario.');
    }
  ...
  async getCurrentShift(userId: string, tenantId: string): Promise<ShiftDocument | null> {
