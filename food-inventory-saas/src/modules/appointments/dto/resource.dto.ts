...
export class CreateResourceDto {
  ...
  @ApiProperty({ description: 'Descripción', example: 'Médico general con 10 años de experiencia', required: false })
  @IsString()
  @IsOptional()
  @MaxLength(1000)
  description?: string;

  @ApiProperty({ description: 'Email', example: 'juan.perez@clinica.com', required: false })
  @IsEmail()
  @IsOptional()
  email?: string;

  @ApiProperty({ description: 'Teléfono', example: '+58 412-1234567', required: false })
  @IsString()
  @IsOptional()
  @MaxLength(20)
  phone?: string;

  @ApiProperty({ description: 'Estado', enum: ['active', 'inactive', 'on_vacation'], default: 'active' })
  @IsEnum(['active', 'inactive', 'on_vacation'])
  @IsOptional()
  status?: string;

  @ApiProperty({ description: 'Color en formato hex', example: '#10B981', required: false })
  @IsString()
  @IsOptional()
  color?: string;

  @ApiProperty({ description: 'Horario de disponibilidad', type: ScheduleDto, required: false })
  @IsObject()
  @ValidateNested()
  @Type(() => ScheduleDto)
  @IsOptional()
...
export class UpdateResourceDto {
  ...
  @ApiProperty({ description: 'Tipo de recurso', enum: ['person', 'room', 'equipment', 'vehicle', 'other'], required: false })
  @IsEnum(['person', 'room', 'equipment', 'vehicle', 'other'])
  @IsOptional()
  type?: string;
  ...
  @ApiProperty({ description: 'Estado', enum: ['active', 'inactive', 'on_vacation'], required: false })
  @IsEnum(['active', 'inactive', 'on_vacation'])
  @IsOptional()
  status?: string;
  ...
  @ApiProperty({ description: 'Fechas no disponibles', type: [UnavailableDateDto], required: false })
  @IsArray()
  @ValidateNested({ each: true })
  @Type(() => UnavailableDateDto)
  @IsOptional()
