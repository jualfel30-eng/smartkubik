...
export class ResourcesController {
  ...
  @ApiResponse({
    status: 200,
    description: "Lista de recursos del tipo especificado",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Lista de recursos compatibles con el servicio",
  })
  getResourcesByService(@Request() req, @Param("serviceId") serviceId: string) {
    return this.resourcesService.getResourcesByService(
      req.user.tenantId,
      serviceId,
    );
  }
