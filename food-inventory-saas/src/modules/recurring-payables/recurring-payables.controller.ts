...
export class RecurringPayablesController {
  constructor(
    private readonly recurringPayablesService: RecurringPayablesService,
  ) {}
  ...
  create(@Request() req, @Body() createDto: CreateRecurringPayableDto) {
    return this.recurringPayablesService.create(
      createDto,
      req.user.tenantId,
      req.user.id,
    );
  ...
  generatePayable(@Request() req, @Param("id") id: string) {
    return this.recurringPayablesService.generatePayable(
      id,
      req.user.tenantId,
      req.user.id,
    );
