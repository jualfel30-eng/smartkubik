...
export class BankReconciliationService {
  constructor(
    ...
    @InjectModel(BankStatementImport.name) private statementImportModel: Model<BankStatementImportDocument>,
  ...
  async importStatement(
    ...
    if (!file) {
      throw new BadRequestException('No file provided for bank statement import');
    }
    ...
    if (!rows.length) {
      throw new BadRequestException('El archivo no contiene movimientos válidos');
    }
    ...
    for (const row of rows) {
      ...
      if (transaction) {
        ...
        transaction.metadata = {
          ...
          bankDescription: row.description ?? transaction.metadata?.bankDescription,
        };
  ...
  async manualReconcile(dto: ManualReconcileDto, tenantId: string) {
    ...
    if (dto.statementTransactionId) {
      transaction.statementTransactionId = new Types.ObjectId(dto.statementTransactionId);
    }
    transaction.metadata = {
      ...(transaction.metadata ?? {}),
    ...
    if (transaction.statementImportId) {
      ...
      if (statementImport) {
        ...
        if (Array.isArray(statementImport.metadata?.unmatched)) {
          statementImport.metadata.unmatched = statementImport.metadata.unmatched.filter(
            (item: any) => {
              const sameAmount =
  ...
  async matchTransaction(
    ...
    reconciliation.summary.matched =
      (reconciliation.summary.matched ?? 0) + 1;
    reconciliation.summary.outstanding =
      (reconciliation.summary.outstanding ?? statement.transactions.length) -
      1;
  ...
  async unmatchTransaction(
    ...
    reconciliation.clearedTransactions = reconciliation.clearedTransactions.filter(
