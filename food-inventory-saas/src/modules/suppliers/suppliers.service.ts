...
export class SuppliersService {
  ...
  async create(
    ...
    try {
      ...
      const verification = await this.supplierModel
        .findById(savedSupplier._id)
        .session(session ?? null)
        .lean();
      if (verification) {
        this.logger.log(
          `READ-AFTER-WRITE CHECK SUCCEEDED for supplier ID: ${savedSupplier._id}`,
        );
      } else {
        this.logger.error(
          `READ-AFTER-WRITE CHECK FAILED for supplier ID: ${savedSupplier._id}`,
        );
      }
      ...
      this.logger.error(
        `Failed to save supplier: ${newSupplier.name}`,
        error.stack,
      );
