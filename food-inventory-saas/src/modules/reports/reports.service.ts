...
export class ReportsService {
  ...
  async generateAccountsReceivableAging(tenantId: string, asOfDate?: string) {
    ...
    const endDate = asOfDate ? moment(asOfDate).endOf('day').toDate() : new Date();
    ...
    for (const order of unpaidOrders) {
      ...
      const daysOverdue = moment(endDate).diff(moment(orderData.createdAt), 'days');

      if (daysOverdue <= 0) { // Should be current if not overdue
