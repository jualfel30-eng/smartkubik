...
export class PermissionsSeed {
  ...
  private readonly permissions: Permission[] = [
    ...
    // Payables
    { name: 'payables_read', description: 'Ver cuentas por pagar', category: 'payables' },
    { name: 'payables_create', description: 'Crear cuentas por pagar', category: 'payables' },
    { name: 'payables_update', description: 'Actualizar cuentas por pagar', category: 'payables' },
    { name: 'payables_delete', description: 'Eliminar cuentas por pagar', category: 'payables' },
  ];
  ...
  async seed(): Promise<void> {
    try {
      ...
      if (existingCount > 0) {
        this.logger.log(`✅ Permissions already seeded (${existingCount} found). Skipping...`);
        return;
      }
      ...
      this.logger.log(`✅ Successfully seeded ${this.permissions.length} permissions`);
    } catch (error) {
