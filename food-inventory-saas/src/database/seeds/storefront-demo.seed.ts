...
export class StorefrontDemoSeed {
  ...
  constructor(
    ...
    @InjectModel(StorefrontConfig.name) private storefrontModel: Model<StorefrontConfigDocument>,
  ...
  async seed() {
    ...
    try {
      ...
      this.logger.log('   E-commerce: http://localhost:3001/tienda-ejemplo.localhost');
      this.logger.log('   Services:   http://localhost:3001/salon-belleza.localhost');
      this.logger.log('');
    } catch (error) {
  ...
  private async cleanDemoData() {
    ...
    await this.tenantModel.deleteMany({
      name: { $in: ['Tienda de Alimentos Premium', 'Salón de Belleza Elegance'] },
    });

    await this.productModel.deleteMany({
      name: { $regex: /^(Harina de Maíz|Aceite de Girasol|Arroz Blanco|Pasta Larga|Azúcar Refinada|Café Molido|Corte de Cabello|Coloración|Manicure)/ },
    });
  }
