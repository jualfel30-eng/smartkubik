...
async function run() {
    ...
    const profilesWithoutUserId: any[] = await employeeProfileModel
    ...
    for (const profile of profilesWithoutUserId) {
        const customer: any = profile.customerId;
        ...
        const customerEmail = customer.email || customer.contacts?.find((c: any) => c.type === 'email')?.value;
        ...
        const user: any = await userModel.findOne({ email: customerEmail }).lean();
