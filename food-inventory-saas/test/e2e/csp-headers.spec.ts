...
describe('Content Security Policy E2E Tests', () => {
  ...
  describe('CSP Header Consistency', () => {
    it('should have consistent CSP across multiple requests', async () => {
      const response1 = await request(app.getHttpServer()).get('/api/v1/health');
      const response2 = await request(app.getHttpServer()).get('/api/v1/health');
    ...
    it('should apply CSP to both GET and POST requests', async () => {
      const getResponse = await request(app.getHttpServer()).get('/api/v1/health');
    ...
    it('should apply CSP to error responses (4xx, 5xx)', async () => {
      // 404 response
      const notFoundResponse = await request(app.getHttpServer()).get('/api/v1/nonexistent');

      expect(notFoundResponse.headers['content-security-policy']).toBeDefined();

      // 401 response
      const unauthorizedResponse = await request(app.getHttpServer()).get('/api/v1/products');

      expect(unauthorizedResponse.headers['content-security-policy']).toBeDefined();
    });
  });
});
