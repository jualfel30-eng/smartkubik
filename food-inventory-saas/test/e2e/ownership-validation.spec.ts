...
describe('Ownership Validation E2E Tests', () => {
  ...
  async function setupTenants() {
    ...
    await mongoConnection.collection('tenants').updateMany(
      { _id: { $in: [new Types.ObjectId(tenant1Id), new Types.ObjectId(tenant2Id)] } },
    ...
    const product1Response = await request(app.getHttpServer())
      ...
      .send({
        ...
        inventoryConfig: { trackLots: false, trackExpiration: false, minimumStock: 10, maximumStock: 100, reorderPoint: 20, reorderQuantity: 50, fefoEnabled: false },
        variants: [{ name: 'Default', sku: `SKU1-VAR-${Date.now()}`, unit: 'unit', unitSize: 1, basePrice: 10.0, costPrice: 5.0 }],
      });
    ...
    const product2Response = await request(app.getHttpServer())
      ...
      .send({
        ...
        pricingRules: { cashDiscount: 0, cardSurcharge: 0, minimumMargin: 0.2, maximumDiscount: 0.5 },
        inventoryConfig: { trackLots: false, trackExpiration: false, minimumStock: 10, maximumStock: 100, reorderPoint: 20, reorderQuantity: 50, fefoEnabled: false },
        variants: [{ name: 'Default', sku: `SKU2-VAR-${Date.now()}`, unit: 'unit', unitSize: 1, basePrice: 20.0, costPrice: 10.0 }],
      });
    ...
    const customer1Response = await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${tenant1Token}`)
      .send({ name: 'Customer', lastName: 'One', email: `customer1-${Date.now()}@example.com`, customerType: 'individual' });
    ...
    const customer2Response = await request(app.getHttpServer())
      .post('/api/v1/customers')
      .set('Authorization', `Bearer ${tenant2Token}`)
      .send({ name: 'Customer', lastName: 'Two', email: `customer2-${Date.now()}@example.com`, customerType: 'individual' });
  ...
  async function cleanupTestData() {
    if (mongoConnection) {
      const tenantObjectIds = [tenant1Id, tenant2Id].filter(Boolean).map((id) => new Types.ObjectId(id));
      if (tenantObjectIds.length > 0) {
        await mongoConnection.collection('users').deleteMany({ email: { $regex: /tenant[12]-.*@example\.com/ } });
        await mongoConnection.collection('tenants').deleteMany({ _id: { $in: tenantObjectIds } });
        await mongoConnection.collection('products').deleteMany({ tenantId: { $in: tenantObjectIds } });
        await mongoConnection.collection('customers').deleteMany({ tenantId: { $in: tenantObjectIds } });
      }
    }
  }
    ...
    it('should allow tenant to delete their own product', async () => {
      const createResponse = await request(app.getHttpServer())
        ...
        .send({
            ...
            pricingRules: { cashDiscount: 0, cardSurcharge: 0, minimumMargin: 0.2, maximumDiscount: 0.5 },
            inventoryConfig: { trackLots: false, trackExpiration: false, minimumStock: 10, maximumStock: 100, reorderPoint: 20, reorderQuantity: 50, fefoEnabled: false },
            variants: [{ name: 'Default', sku: `DEL-VAR-${Date.now()}`, unit: 'unit', unitSize: 1, basePrice: 15.0, costPrice: 7.0 }],
        });
